...
// Rotating word + its matching product image.
const SHOWCASE = [
  ...
  { word: 'Banners',   src: '/P4.jpg', bg: 'from-brand-400 to-green-500' },
]

const ROTATE_MS = 3600

// Inline styles ensure rounding is applied on the very first paint,
// independent of CSS-class load order or transform/overflow interactions.
...
// Decorative lime asterisk that gently spins.
function SpinningAsterisk({ className = '', size = 28, duration = 14 }) {
  return (
    <motion.span
      ...
      className={`text-brand-500 inline-flex ${className}`}
    >
...
export default function Hero() {
  ...
  return (
    <section id="services" className="relative bg-white overflow-hidden pt-20 sm:pt-24">
      {/* Single lime spotlight — LEFT on mobile, RIGHT on md+ */}
      <motion.div
        aria-hidden
        animate={{ x: [0, 40, 0], y: [0, 30, 0] }}
        transition={{ duration: 22, repeat: Infinity, ease: 'easeInOut' }}
        className="pointer-events-none absolute -top-40 -left-32 md:left-auto md:-right-32 w-[32rem] h-[32rem] md:w-[44rem] md:h-[44rem] rounded-full bg-brand-300/45 blur-[120px] md:blur-[140px]"
      />
      ...
      <div className="relative max-w-[88rem] mx-auto px-4 sm:px-6 lg:px-10 pt-10 sm:pt-14 pb-20 sm:pb-28 grid lg:grid-cols-12 gap-10 lg:gap-12 items-center">
        ...
        <div className="relative z-10 lg:col-span-7">
          ...
          <h1 className="mt-5 text-5xl sm:text-6xl md:text-7xl lg:text-[6rem] xl:text-[7rem] font-black tracking-tighter text-slate-900 leading-[0.95]">
            ...
            {/* Rotating word with lime highlight bar behind it */}
                ...
                <AnimatePresence mode="wait">
                  ...
                  >
                    <span className="relative">
                      {/* lime highlight bar behind text */}
        ...
        {/* Right — card stack, fully responsive (stable keys = buttery smooth) */}
        <div className="relative lg:col-span-5">
          ...
          >
            {/* Every item gets a stable component. Only its transform/opacity
                animates as its depth in the stack changes — no remounts,
                no image reloads → smooth on mobile too. */}
            {SHOWCASE.map((item, idx) => {
              // Distance from current top of stack (looping).
              ...
              const target = inStack
                ...
                : {
                    // Items not on display sit just below the stack, invisible,
                    // ready to fade in when they cycle back to depth 2.
                    y: 60,
                    x: 0,
                    rotate: 6,
                    scale: 0.86,
              ...
              return (
                <motion.div
                  key={item.word /* STABLE — never remounts */}
                  initial={false}
                  animate={target}
                  ...
                  style={{
                    ...
                    borderRadius: '2rem',
                    overflow: 'hidden',
                    // clip-path stays glued to children even under transforms — fixes
                    // the first-paint flash where corners look square.
                ...
                >
                  ...
                  {/* Overlay content fades in only on the active top card */}
                  <motion.div
                  ...
                  >
                    ...
                    <div className="absolute top-5 left-5 pointer-events-auto">
                      <div className="inline-flex items-center gap-1.5 bg-white/95 backdrop-blur-md rounded-full px-3.5 py-1.5 text-xs sm:text-sm font-semibold text-slate-900 shadow-md">
                        {item.word}
                    ...
                    {/* Live indicator */}
                    <div className="absolute top-5 right-5 flex items-center gap-2 bg-black/45 backdrop-blur-md text-white text-[11px] sm:text-xs font-medium rounded-full px-3 py-1.5">
            ...
            {/* Premium Quality chip — outside, doesn't move with the stack */}
            <motion.div
